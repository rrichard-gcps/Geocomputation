...
metro_map <- ggplot() +
  ...
  geom_sf(
    data = st_transform(metro_20_bounds, 2240),
    fill = NA,
    color = pal$ring,
    lwd = 0.4
  ) + # lighter ring
  ...
  geom_text_repel(
    ...
    color = pal$hi_line,
    bg.color = pal$halo,
    bg.r = 0.12, # ggrepel >=0.9.2 halo
    segment.color = pal$hi_line,
    segment.size = 0.5,
...
legend_plot <- ggplot(leg_df, aes(x = (col - 1) * 1.0, y = -row)) +
  ...
  annotate(
    ...
    x = 0,
    y = 1.4,
    label = "OTHER METRO SCHOOL SYSTEMS",
    hjust = 0,
    family = "Montserrat",
    fontface = "bold",
    color = pal$faint,
...
ring_geom <- st_buffer(
  st_centroid(hi_union),
  dist = as.numeric(max(st_distance(
    st_centroid(hi_union),
    st_cast(hi_union, "POINT")
  ))) *
    1.25
)
ga_inset <- ggplot() +
...
# (placed directly via draw_label in the composition below for reliable
#  left-alignment; see draw_label calls in the ggdraw section)
...
day3_map <- ggdraw() +
  ...
  draw_label(
    "Location map highlighting Gwinnett County, Atlanta Public Schools & Henry County",
    x = 0.03,
    y = 0.925,
    hjust = 0,
...
ggsave(
  day3_map,
  filename = "gcps_aps_henry_map2.png",
  type = "cairo",
  scale = 0.75,
  width = 18,
  height = 12,
