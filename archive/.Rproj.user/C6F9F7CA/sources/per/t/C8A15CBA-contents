...
credits <- tibble(
  label = c(
    ...
    <b>Day 5:</b> Journeys<br>
    <b>Tool:</b> R <br>
    <b>Created By:</b> Roland Richard<br>
    <span style='font-size:10pt; color:#CFCFCF;'><b>Data Sources:</b> The Atlantic Coast Conference (2024)</span>"
  )
)
...
p <- ggplot() +
  ...
  geom_text_repel(data = teams_sf,
                  ...
                  max.overlaps = Inf, size = 4, box.padding = 2, force = 10, point.padding = 1.5, nudge_x = 0.3, nudge_y = 0.3, family = headline_font_family, fontface = "bold", hjust = 0.5) +
  ...
  theme_void() +
  geom_textbox(data = credits, aes(label = label), x = -Inf, y = -Inf, hjust = -0.1, vjust = -0.1, box.color = NA, fill = NA, width = unit(3, 'inches'), family = body_font_family, size = 3, color = '#CFCFCF') +
  theme(
    plot.background = element_rect(fill = acc_blue, color = NA),
    panel.background = element_rect(fill = acc_blue, color = NA),
    plot.margin = margin(20, 20, 20, 20),
    text = element_text(color = acc_blue, face = "plain", size = 14, family = body_font_family),
    plot.title = element_text(size = 24, face = "bold", color = "#FFFFFF", family = headline_font_family, hjust = 0, margin = margin(t = 10, b = 10)),
    plot.subtitle = element_text(size = 14, face = "plain", color = "#FFFFFF", family = body_font_family, hjust = 0.5, margin = margin(t = 5, b = 10))
  ) +
  labs(
    title = "ACC Basketball Team Travel to the 2024-25 ACC Tournament",
    subtitle = paste("Total Cumulative Miles Traveled: ", round(sum(teams$distance_to_charlotte)), " miles"),
    x = "", y = ""
  )

# Save the map
ggsave("us_map_acc_travel.png", p, width = 12, height = 10, units = "in", limitsize = FALSE)
